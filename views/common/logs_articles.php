...
// Function Log Article Action
function logger($log){
    if(!file_exists('../../log_articles.txt')){
        file_put_contents('../../log_articles.txt', '');
    }
    $ip = $_SERVER['REMOTE_ADDR'];
    date_default_timezone_set('France/Paris');
    $time = date('m/d/y h:i', time());

    // 1ere solution avec fopen('../../log_articles.txt', 'a+')
    $file_a = fopen('../../log_articles.txt', 'a+');

    //2 ieme solution avec file_put_contents
    // $contents = file_put_contents('../../log_articles.txt');
    // file_put_contents('../../log_articles.txt', $contents, "\n", FILE_APPEND);
